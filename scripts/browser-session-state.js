...
function createSessionStore(initial = {}) {
  ...
  function syncFromLiveThreads(threads, options = {}) {
    ...
    // This method backs the swipe target / Open sessions UI. Only live bridge rows
    // belong here; archived history must stay in the thread list and out of open sessions.
    ...
    const previousActive =
      state.openSessions.find((session) => session.key === state.activeSessionKey && session.threadId) || state.resumeCandidateSession || null;
    ...
    if (
      ...
      !state.openSessions.some((session) => session.key === state.activeSessionKey) &&
      state.resumeCandidateSession?.key !== state.activeSessionKey
    ) {
      state.activeSessionKey = "";
    }
  ...
  function restoreFromLiveThreads(threads) {
    ...
    const saved =
      state.openSessions.find((session) => session.key === state.activeSessionKey && liveIds.has(session.threadId)) ||
      state.openSessions.find((session) => liveIds.has(session.threadId)) ||
      state.resumeCandidateSession ||
      null;
