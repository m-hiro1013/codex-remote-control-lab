...
- Use `PHONE_DEBUG_NO_TOKEN=1` or local `.env` `ENV=debug` only as a localhost debugging exception. It binds the bridge to `127.0.0.1` and must not be exposed to LAN devices, tunnels, or shared networks.
