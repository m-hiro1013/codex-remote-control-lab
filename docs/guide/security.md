...
`PHONE_DEBUG_NO_TOKEN=1` is a localhost-only debugging exception. `ENV=debug` in a local `.env` file enables the same mode for repeated local debugging. In that mode the bridge binds to `127.0.0.1` and does not create or require a token.

`ENV=debug-lan` is the explicit tokenless LAN debug mode. It binds the bridge to `0.0.0.0` and prints tokenless LAN URLs. Use it only on a trusted private network you control, and never through public tunnels or raw port forwards.
