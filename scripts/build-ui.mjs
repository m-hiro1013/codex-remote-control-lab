...
async function buildBrowserSessionState({ check }) {
  const tmpDir = path.join(repoRoot, "node_modules/.cache/codex-remote-control-lab");
  const outJs = check ? path.join(tmpDir, `browser-session-state-${process.pid}.js`) : browserSessionStateOutputPath;
  await fs.mkdir(path.dirname(outJs), { recursive: true });
  await esbuild.build({
    entryPoints: [browserSessionStatePath],
    bundle: true,
    format: "iife",
    globalName: "CodexSessionBrowserState",
    target: "es2020",
    outfile: outJs,
    sourcemap: false,
    logLevel: "silent",
  ...
  try {
    const [currentJs, nextJs] = await Promise.all([
      fs.readFile(browserSessionStateOutputPath, "utf8"),
      fs.readFile(outJs, "utf8"),
    ]);
    if (currentJs !== nextJs) {
      console.error("public/browser-session-state.js is out of date. Run npm run build:ui.");
      process.exit(1);
    }
  } finally {
