...
既定では page、API、WebSocket に token が必要です。host だけで UI を確認する場合は `PHONE_DEBUG_NO_TOKEN=1 npm run phone` または `.env` の `ENV=debug` を使うと、bridge は `127.0.0.1` に bind され、token なし URL を表示します。信頼できる private LAN に token なしで出す場合だけ、`ENV=debug-lan` を使います。
