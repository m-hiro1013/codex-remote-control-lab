...
- Use `PHONE_DEBUG_NO_TOKEN=1` or local `.env` `ENV=debug` as the localhost debugging exception. Use `ENV=debug-lan` only for intentional tokenless LAN debugging on a trusted private network.
